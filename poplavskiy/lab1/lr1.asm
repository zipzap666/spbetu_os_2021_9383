...
PRINT_TYPE_PC PROC NEAR	;определение и вывод типа ПК
		push 	AX
		push	BX
		push	DI
...
VERSION_DOS PROC NEAR	;определение версии системы
		push	AX
		push	SI
		mov 	SI, OFFSET VERS
		add 	SI, 10h
...
OEM_NUM PROC NEAR	;определение серийного номера OEM
		push	AX
		push	BX
		push	SI
		mov		AL, BH
...
USER_NUM PROC NEAR	;определение серийного номера пользователя
		push 	CX
		push 	DI
		push 	AX
		mov 	DI, OFFSET USER
